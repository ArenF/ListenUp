...
### 1. UI/UX 개선 - 실시간 반응 시스템 🎨

**목표**: 게임 진행 중 실시간으로 플레이어 상태를 시각적으로 표현하여 UX 향상

#### 구현 내용
- [ ] **정답 시 피드백**
  - 정답을 맞춘 유저의 네임바가 초록색으로 물들기
  - 애니메이션: 부드러운 색상 전환 (0.3s ease)

- [ ] **오답 시 피드백**
  - 틀린 유저의 네임바가 붉은색으로 표시
  - 애니메이션: shake 효과 추가 (0.5s)

- [ ] **답안 제출 애니메이션**
  - 정답을 입력하면 네임바가 살짝 튀어오르는 애니메이션
  - Bounce 효과: `transform: translateY(-10px)` → `translateY(0)`

- [ ] **점수 증가 애니메이션**
  - 점수가 올라갈 때 숫자 카운팅 애니메이션
  - +점수 텍스트가 떠올라가는 효과

#### 기술 스택
- Svelte 5 transitions/animations
- CSS animations (keyframes)
- Socket.IO 이벤트 활용 (`answer-submitted`, `score-updated`)
...
| 작업 | 우선순위 | 예상 기간 | 담당 |
|------|---------|----------|------|
| 1. UI/UX 실시간 반응 | 높음 | 2-3일 | TBD |
| 2. 힌트 시스템 | 높음 | 3-4일 | TBD |
| 3. 재생 구간 설정 | 중간 | 2-3일 | TBD |
| 4. 정답 영상 재생 | 중간 | 2-3일 | TBD |
